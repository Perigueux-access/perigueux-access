...
const DEFAULT_PHOTOS = {
  ...
  parc: [
    'https://upload.wikimedia.org/wikipedia/commons/thumb/1/17/P%C3%A9rigueux_-_Jardin_des_Ar%C3%A8nes_%281%29.jpg/640px-P%C3%A9rigueux_-_Jardin_des_Ar%C3%A8nes_%281%29.jpg',
    'https://images.unsplash.com/photo-1568515387631-8b650bbcdb90?w=600&q=80',
    'https://images.unsplash.com/photo-1445964047600-cdbdb873673d?w=600&q=80',
  ],
  balade: [
    'https://upload.wikimedia.org/wikipedia/commons/thumb/8/8e/Isle_%28rivi%C3%A8re%29_-_Boulazac_Isle_Manoire.jpg/640px-Isle_%28rivi%C3%A8re%29_-_Boulazac_Isle_Manoire.jpg',
    'https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1?w=600&q=80',
  ],
  culture: [
    'https://upload.wikimedia.org/wikipedia/commons/thumb/e/e3/Perigueux_-_Cathedrale_Saint-Front_%281%29.jpg/640px-Perigueux_-_Cathedrale_Saint-Front_%281%29.jpg',
    'https://upload.wikimedia.org/wikipedia/commons/thumb/6/6e/Perigueux_-_Tour_Mataguerre.jpg/480px-Perigueux_-_Tour_Mataguerre.jpg',
    'https://images.unsplash.com/photo-1564399579883-451a5d44ec08?w=600&q=80',
  ],
  commerce: [
    'https://images.unsplash.com/photo-1542838132-92c53300491e?w=600&q=80',
    'https://images.unsplash.com/photo-1507914997316-b9a2a038a7ec?w=600&q=80',
  ],
  toilettes: [
...
// ======================================================
// ============================================================
// 🗄️ STORAGE — localStorage global (fonctionne sur Vercel)
// Déclaré hors composant pour éviter les problèmes de closure
// ============================================================
const storage = {
  get: (key) => {
    try {
...
export default function PerigueuxAccess() {
  ...
  // Masquer l'écran de chargement HTML dès que React est monté
  useEffect(() => {
    const ls = document.getElementById('loading-screen');
    if (ls) {
      ls.style.transition = 'opacity 0.3s ease';
      ls.style.opacity = '0';
      setTimeout(() => { if (ls.parentNode) ls.parentNode.removeChild(ls); }, 350);
    }
  }, []);
  ...
  useEffect(() => {
    (async () => {
      ...
      const v = storage.get('seed_version');
      ...
      if (currentVersion !== SEED_VERSION) {
        ...
        const pls = storage.get('places');
        if (pls) {
          try { setPlaces(JSON.parse(pls.value)); } catch(e) { setPlaces(SEED_PLACES); }
        } else {
          setPlaces(SEED_PLACES);
        }
      }
      ...
      if (lb) {
        try { setLeaderboard(JSON.parse(lb.value)); } catch(e) {}
      }

      const onb = storage.get('onboarding_seen');
  ...
  useEffect(() => {
    if (loading) return;
    storage.set('places', JSON.stringify(places));
  }, [places, loading]);
    ...
    const onKey = (e) => {
      if (e.key === 'Escape') {
        if (sosActive) closeSOS();
          ...
          storage.set('onboarding_seen', 'yes');
  ...
  const disableLocation = () => {
    ...
    if (userMarkerRef.current && mapInstance.current) {
      try { mapInstance.current.removeLayer(userMarkerRef.current); } catch(e) {}
      userMarkerRef.current = null;
    }
    // Supprimer le cercle de rayon
    if (window._userRadiusCircle && mapInstance.current) {
      try { mapInstance.current.removeLayer(window._userRadiusCircle); } catch(e) {}
      window._userRadiusCircle = null;
    }
    // Remettre la vue sur Périgueux
    if (mapInstance.current) {
      mapInstance.current.setView([45.1846, 0.7229], 14, { animate: true });
    }
  ...
  const filteredPlaces = useMemo(() => {
    let list = places.filter(p => {
      ...
      if (filters.search) {
        ...
        // Cherche dans : nom, adresse, quartier, type, label du type, description
        const typeLabel = normalize(TYPES[p.type]?.label || '');
        const typeName = normalize(p.type || '');
        const desc = normalize(p.description || '');
        const reviewText = normalize((p.reviews || []).map(r => r.text).join(' '));
        if (
          !normalize(p.name).includes(q) &&
          !normalize(p.address).includes(q) &&
          !normalize(p.quartier).includes(q) &&
  ...
  // ✅ Ref toujours à jour — DOIT être avant refreshMarkers
  const filteredPlacesRef = useRef([]);

  const refreshMarkers = (placesToShow) => {
    if (!mapInstance.current || !window.L) return;
    const list = placesToShow !== undefined ? placesToShow : filteredPlacesRef.current;
  ...
  // Sync la ref + refresh marqueurs quand filteredPlaces change
  useEffect(() => {
    filteredPlacesRef.current = filteredPlaces;
    if (view === 'map') refreshMarkers(filteredPlaces);
  }, [filteredPlaces, profile.favorites]);

  // Retour sur la carte — juste rafraîchir les marqueurs
  useEffect(() => {
    if (view !== 'map' || !mapInstance.current) return;
    refreshMarkers(filteredPlacesRef.current);
  }, [view]);

  // ============================================================
  // 🗺️ LEAFLET — déclaré APRÈS filteredPlacesRef et refreshMarkers
  // ============================================================
  ...
  const initMap = () => {
    ...
    mapInstance.current = L.map(container, {
      zoomControl: true, dragging: true, tap: true,
      tapTolerance: 15, touchZoom: true,
  ...
  // Charger Leaflet après le chargement des données
  useEffect(() => {
    if (loading) return;
    const load = async () => {
      ...
      if (!window.L) {
        ...
        }).catch(() => showToast('⚠️ Carte indisponible'));
      }
      if (!window.L) return;
  ...
  // Maj tuiles quand thème change
  useEffect(() => {
    if (!mapInstance.current || !window.L) return;
    mapInstance.current.eachLayer(l => {
      if (l instanceof window.L.TileLayer) mapInstance.current.removeLayer(l);
    });
    window.L.tileLayer(
      dark
        ? 'https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png'
  ...
  return (
    <div style={{
      ...
      {/* ============================================================ */}
      {/* 🗺️ CARTE — position fixed, toujours dans le DOM             */}
      {/* ============================================================ */}
      <div
        ref={mapRef}
        style={{
          position: 'fixed',
          top: 68, left: 0, right: 0, bottom: 80,
          zIndex: 2,
          background: '#e8e0d0',
        }}
      />
      ...
      {/* ============================================================ */}
      {/* PAGES — position fixed par-dessus la carte                   */}
      {/* ============================================================ */}

      {/* Vue carte : filtres flottants par-dessus Leaflet */}
      {view === 'map' && (
        <MapView
      ...
      {/* Pages secondaires — couvrent toute l'écran au-dessus de la carte */}
      {view !== 'map' && (
        <div style={{
          position: 'fixed',
          top: 68, left: 0, right: 0, bottom: 80,
      ...
      <nav style={{
        ...
        zIndex: 1001, // Au-dessus des marqueurs Leaflet (zIndexOffset: 1000)
        boxShadow: '0 -2px 12px rgba(0,0,0,0.08)',
        backdropFilter: 'blur(10px)',
        WebkitBackdropFilter: 'blur(10px)',
...
// VUE CARTE
// ======================================================
function MapView({ theme, fontDisplay, filters, setFilters, showFilters, setShowFilters, places, filteredPlaces, onSelectPlace, userLocation, locationStatus, onRequestLocation, onDisableLocation, profile, onToggleFavorite, onRecenter, dark }) {
  ...
  return (
    <div style={{
      ...
      {/* Panneau filtres/recherche en haut */}
      <div style={{
        padding: '14px 20px 10px',
        background: dark ? 'rgba(26,22,18,0.92)' : 'rgba(245,239,224,0.92)',
        ...
        <div style={{display:'flex',gap:8,marginTop:10,overflowX:'auto',paddingBottom:2}}>
          <button
            onClick={userLocation ? onDisableLocation : onRequestLocation}
            aria-label={userLocation ? 'Désactiver ma position' : 'Me localiser'}
            style={{
            ...
            <Locate size={14}/>
            {locationStatus === 'loading' ? 'Localisation…' : userLocation ? '📍 Désactiver' : 'Me localiser'}
          </button>
      ...
      {/* Filtres */}
      {showFilters && (
        <div style={{padding: '14px 20px', background: theme.surfaceAlt, borderBottom: `1px solid ${theme.border}`, pointerEvents: 'auto'}}>
      ...
      {filteredPlaces.length === 0 && (
        <div style={{
          ...
          margin: '0 20px', pointerEvents: 'auto',
        }}>
      ...
      {/* Bouton recentrer flottant — pointerEvents auto */}
      {userLocation && (
        <button onClick={onRecenter} aria-label="Recentrer" style={{
          position: 'fixed', bottom: 88, right: 14, zIndex: 20,
...
function LeaderboardView({ theme, fontDisplay, profile, leaderboard, setLeaderboard }) {
  ...
  const publishScore = async () => {
    try {
      ...
      storage.set('leaderboard', JSON.stringify(updated));
